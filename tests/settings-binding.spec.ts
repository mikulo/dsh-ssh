...
function fakeContext(options: { familyBinder?: object; mirror: FakeMirror; forms: Map<string, ConfigForm<unknown>>; asked: string[] }): ClientContext {
  ...
  // The reader resolves both seats through ctx.get(), so the fake answers them
  // there as well as on the property face.
  return {
    get: (name: string) => (name === 'webUiSettings' ? options.familyBinder : name === 'configForms' ? configForms : undefined),
    configForms,
